...
export default class ShaanLootSheetSR extends ActorSheet {
    ...
    static get defaultOptions() {
        ...
        return options.classes = [...options.classes, "character"], options.width = 700, options.height = 600, options.scrollY.push(".sheet-body"), options
    }
    get template(){
    ...
    async getData(options = this.options) {
        ...
        const actorData = this.actor.toObject(!1),
            ...
            sheetData.enrichedGMnotes = await TextEditor.enrichHTML(getProperty(this.actor.system, "details.description"), {async: true})
    ...
    activateListeners(html) {
        ...
        html.find(".split-coins").removeAttr("disabled").on("click", (event) => this.distributeCoins(event));
        html.find(".loot-npcs").removeAttr("disabled").on("click", (event) => this.lootNPCs(event));
