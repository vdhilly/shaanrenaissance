...
export class CompendiumBrowserBestiaryTab extends CompendiumBrowserTab {
    constructor(browser) {
        super(browser), this.tabName = "bestiary", this.templatePath = "systems/shaanrenaissance/templates/compendium-browser/partials/bestiary.hbs", this.index = ["img", "system.source", "system.general.classe", "system.general.role","system.general.milieu"], this.searchFields = ["name"], this.storeFields = ["type", "name", "img", "uuid", "source"], this.filterData = this.prepareFilterData()
    }
    async loadData() {
        console.debug("SR System | Compendium Browser | Started loading Bestiary actors");
        const bestiaryActors = [],
            sources = new Set,
            indexFields = ["img", "system.source", "system.general.classe", "system.general.role","system.general.milieu"];
        for await (const {
            pack,
            index
            ...
            for (const actorData of index)
                if ("Personnage" === actorData.type || "PNJ" === actorData.type || "Créature" === actorData.type) {
                    ...
                    source && (sources.add(source), actorData.system.source = (0, sluggify)(source)), bestiaryActors.push({
                        type: actorData.type,
                        name: actorData.name,
                        img: actorData.img,
    ...
    filterIndexData(entry) {
        ...
        return (!(checkboxes.type.selected.length && !checkboxes.type.selected.includes(entry.type)) && !(checkboxes.class.selected.length && !checkboxes.class.selected.includes(entry.class)) && !(checkboxes.role.selected.length && !checkboxes.role.selected.includes(entry.role)) && !(checkboxes.source.selected.length && !checkboxes.source.selected.includes(entry.source)))
    }
    prepareFilterData() {
        return {
            checkboxes: {
