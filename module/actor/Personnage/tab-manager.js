export class PersonnageSheetTabManager {
  constructor(actor, link) {
    (this.actor = actor),
      (this.link = link),
      foundry.applications.handlebars.renderTemplate(`systems/shaanrenaissance/templates/actors/${actor.type.replace("Réseau", "Reseau")}/manage-tabs.hbs`).then((template) => {
  ...
  onReady(tooltip) {
    ...
    const tabVisibility = this.actor.flags.shaanRenaissance.sheetTabs,
      ...
      tabs = null !== (_a = null == nav ? void 0 : nav.querySelectorAll("a.item[data-tab]")) && void 0 !== _a ? _a : [];
    for (const tab of Array.from(tabs)) {
      ...
      (tooltip.querySelector(selector).checked = tabVisibility[tabName]),
        tab.classList.contains("hidden") && (tab.classList.remove("hidden"), tab.classList.add("to-hide"));
    }
    const checkboxes = Array.from(tooltip.querySelectorAll('input[type="checkbox"]'));
    for (const checkbox of checkboxes) this.handleOnChange(checkbox, checkboxes);
  }
  handleOnChange(checkbox, checkboxes) {
    checkbox.addEventListener("change", async () => {
      ...
      if (checkbox.checked) {
        ...
        await this.actor.update({ [`flags.shaanRenaissance.sheetTabs.-=${tabName}`]: null }, { render: false });
      } else {
        tab?.classList.add("to-hide");
        await this.actor.update({ [`flags.shaanRenaissance.sheetTabs.${tabName}`]: false }, { render: false });
      }
  ...
  onClose() {
    ...
    const tabs = Array.from(
      null !==
        (_b = null === (_a = this.link.closest("nav")) || void 0 === _a ? void 0 : _a.querySelectorAll("a.item[data-tab]")) &&
        void 0 !== _b
        ? _b
        : []
    );
    for (const tab of tabs) tab.classList.contains("to-hide") && (tab.classList.remove("to-hide"), tab.classList.add("hidden"));
  }
}
