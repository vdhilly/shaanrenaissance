...
export class ShaaniSheetSR extends ActorSheetSR {
  ...
  processDomains(sheetData) {
    const shaandars = sheetData.system.details.shaandars;

    for (const key in shaandars) {
      if (shaandars[key].length === 1) {
        sheetData.system.skills[key].rank = shaandars[key][0].system.skills[key].rank;
        ...
        sheetData.system.skills[key].rank = rankValue;
      } else {
        let ranks = [];
  ...
  processSkills(sheetData) {
    ...
    const beforeSkills = JSON.parse(JSON.stringify(sheetData.system.skills));
    ...
    function updateSheetData(sheetData, members) {
      members.forEach((member) => {
        Object.keys(member.skills).forEach((category) => {
          Object.keys(sheetData.system.skills[category].specialisations).forEach((skill) => {
            ...
            const sheetBonus = sheetData.system.skills[category].specialisations[skill].bonus;
            const sheetAcquis = sheetData.system.skills[category].specialisations[skill].acquis;

            // Compare and update if member has better bonus or acquis
            if (memberBonus > sheetBonus) {
              sheetData.system.skills[category].specialisations[skill].bonus = memberBonus;
            }

            if (memberAcquis > sheetAcquis) {
              sheetData.system.skills[category].specialisations[skill].acquis = memberAcquis;
            }
          });
        });
      });
  ...
  defineTrihnMax(sheetData) {
    const soucheEspritKey = sheetData.system.soucheEsprit;
    const soucheEsprit = sheetData.members[soucheEspritKey];
    const soucheAmeKey = sheetData.system.soucheAme;
    const soucheAme = sheetData.members[soucheAmeKey];
    const soucheCorpsKey = sheetData.system.soucheCorps;
  ...
  prepareMembers() {
    return this.actor.members.map((actor) => {
      ...
      const restricted = !(game.settings.get("shaanrenaissance", "showShaaniStats") || observer);
        ...
        htmlQuery(memberElem, "a[data-action=remove-member]")?.addEventListener("click", async (event) => {
          const confirmed = event.ctrlKey
            ...
            : await Dialog.confirm({
                title: game.i18n.localize("SR.Actor.Shaani.RemoveMember.Title"),
                content: game.i18n.localize("SR.Actor.Shaani.RemoveMember.Content"),
              });
          if (confirmed && actor) {
        ...
        const data = {
          ...
          buttons: {
            normal: {
              ...
              callback: (html) => resolve(_processShaandarOptions(html[0].querySelector("form"))),
            },
            cancel: {
          ...
          newMembers.forEach((newMember) => {
            ...
            if (memberExistsElsewhere) {
              Object.keys(existingShaandars).forEach((otherKey) => {
                if (otherKey !== key) {
                  existingShaandars[otherKey] = existingShaandars[otherKey].filter(
                    (otherMember) => otherMember._id !== newMember._id
                  );
                }
              });
            }
