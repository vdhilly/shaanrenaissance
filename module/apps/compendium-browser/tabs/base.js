...
export class CompendiumBrowserTab {
  ...
  hasAllIndexFields(data, indexFields) {
    for (const field of indexFields) if (void 0 === foundry.utils.getProperty(data, field)) return !1;
    return !0;
  }
  ...
  async createRollTable() {
    ...
    Dialog.confirm({
      ...
      yes: async ($html) => {
        const html = $html[0];
        const name = html.querySelector("input[name=name]")?.value || game.i18n.localize("SR.CompendiumBrowser.Title");
  ...
  async addToRollTable() {
    ...
    Dialog.confirm({
      ...
      yes: async ($html) => {
        ...
        await table.createEmbeddedDocuments("TableResult", this.getRollTableResults({ initial: table.results.size, weight }));
        table?.sheet.render(true);
      },
    });
  }
