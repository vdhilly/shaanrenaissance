...
var _CompendiumBrowserTab_domParser, __classPrivateFieldGet = this && this.__classPrivateFieldGet || function(receiver, state, kind, f) {
...
export class CompendiumBrowserTab {
    constructor(browser) {
        this.indexData = [], this.isInitialized = !1, this.totalItemCount = 0, this.scrollLimit = 100, _CompendiumBrowserTab_domParser.set(this, new DOMParser), this.searchFields = [], this.storeFields = [], this.browser = browser
    }
    async init() {
        await this.loadData(), this.searchEngine = new MiniSearch({
        ...
        }), this.searchEngine.addAll(this.indexData), this.defaultFilterData = deepClone(this.filterData), this.isInitialized = !0
    }
    getIndexData(start) {
        if (!this.isInitialized) return ui.notifications.error(`Compendium Browser Tab "${this.tabName}" is not initialized!`);
        const currentIndex = (() => {
        ...
        return this.totalItemCount = currentIndex.length, currentIndex.slice(start, this.scrollLimit)
    }
    resetFilters() {
        this.filterData = deepClone(this.defaultFilterData)
    }
    async renderResults(start) {
        if (!this.templatePath) return ui.notifications.error(`Tab "${this.tabName}" has no valid template path`);
        ...
        for (const entry of indexData) {
            const htmlString = await renderTemplate(this.templatePath, {
                ...
                html = __classPrivateFieldGet(this, _CompendiumBrowserTab_domParser, "f").parseFromString(htmlString, "text/html");
    ...
    sortResult(result) {
        const {
            order
        } = this.filterData, lang = game.i18n.lang, sorted = result.sort(((entryA, entryB) => {
            switch (order.by) {
                case "name":
    ...
    generateCheckboxOptions(configData, sort = !0) {
        const localized = Object.entries(configData).reduce(((result, [key, label]) => ({
            ...result,
            [key]: game.i18n.localize(label)
        })), {});
        return Object.entries(sort ? this.sortedConfig(localized) : localized).reduce(((result, [key, label]) => ({
            ...result,
            [key]: {
                label,
                selected: !1
    ...
    generateSourceCheckboxOptions(sources) {
        return [...sources].sort().reduce(((result, source) => ({
            ...result,
            [(0, sluggify)(source)]: {
                label: source,
                selected: !1
    ...
    sortedConfig(obj) {
        return Object.fromEntries([...Object.entries(obj)].sort(((entryA, entryB) => entryA[1].localeCompare(entryB[1], game.i18n.lang))))
    }
    ...
    async createRollTable() {
        ...
        Dialog.confirm({
            ...
            yes: async ($html) => {
                ...
                const weight = Number(html.querySelector("input[name=weight]")?.value) || 1;
    ...
    async addToRollTable() {
        ...
        Dialog.confirm({
            title: game.i18n.localize("SR.CompendiumBrowser.RollTable.SelectTableTitle"),
            content,
            yes: async ($html) => {
                const html = $html[0];
                const option = html.querySelector("select[name=roll-table]")?.selectedOptions[0];
                if (!option) return;
                const weight = Number(html.querySelector("input[name=weight]")?.value) || 1;
