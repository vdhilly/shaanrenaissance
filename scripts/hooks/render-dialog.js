  ...
  listen: () => {
    Hooks.on("renderDialog", (_dialog, $html) => {
      ...
      if (element.classList.contains("dialog-item-create")) {
        ...
        select &&
          (select.append(
            extractOptGroup(select, categories.Acquis, [
              "Armement",
              "Armimale",
              "Manuscrit",
              "Artefact",
          ...
          select.append(
            extractOptGroup(select, categories.Personnage, [
              "Race",
              "Peuple",
              "Caste",
              "Métier",
...
function extractOptGroup(select, label, types) {
  const filtered = [
      ...select.querySelectorAll(":scope > option").values(),
    ].filter((option) => !types || types.includes(option.value)),
