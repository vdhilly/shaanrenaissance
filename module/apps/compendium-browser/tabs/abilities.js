...
export class CompendiumBrowserAbilitiesTab extends CompendiumBrowserTab {
  constructor(browser) {
    super(browser),
      (this.tabName = "abilities"),
      (this.templatePath =
        "systems/shaanrenaissance/templates/compendium-browser/partials/abilities.hbs"),
      (this.searchFields = ["name"]),
      (this.storeFields = ["type", "name", "img", "uuid", "source"]),
      (this.index = [
        "img",
        "system.pouvoirType.value",
        "system.pouvoir.value",
        "system.rang.value",
        "system.source.value",
      ]),
  ...
  async loadData() {
    console.debug(
      "SHAAN RENAISSANCE | Compendium Browser | Started loading abilities"
    );
    const abilities = [],
      indexFields = [
        "img",
    ...
    for await (const { pack, index } of this.browser.packLoader.loadPacks(
      "Item",
      this.browser.loadedPacks("abilities"),
      indexFields
    )) {
      console.debug(
      ...
      for (const abilitiesData of index)
        if ("Pouvoir" === abilitiesData.type) {
          if (!this.hasAllIndexFields(abilitiesData, indexFields)) {
            console.warn(
              `Ability '${abilitiesData.name}' does not have all required data fields. Consider unselecting pack '${pack.metadata.label}' in the compendium browser settings.`
            );
            continue;
          }
          ...
          source && sources.add(source),
            abilities.push({
              ...
              domain: abilitiesData.system.pouvoir.value
                .replace("de ", "")
                .replace("Astuce ", "")
                .replace("Création d'", "")
                .replace("Privilège ", "")
    ...
    (this.indexData = abilities),
      ...
      (this.filterData.checkboxes.types.options = this.generateCheckboxOptions(
        CONFIG.shaanRenaissance.abilitiesTypes
      )),
      (this.filterData.checkboxes.rangs.options = this.generateCheckboxOptions(
        CONFIG.shaanRenaissance.pouvoirs.pouvoirRank
      )),
      (this.filterData.checkboxes.source.options =
        this.generateSourceCheckboxOptions(sources)),
      console.debug(
        "SHAAN RENAISSANCE | Compendium Browser | Finished loading abilities"
      );
  }
  // A changer
  filterIndexData(entry) {
    ...
    return (
      ...
      !(
        checkboxes.rangs.selected.length &&
        !checkboxes.rangs.selected.includes(entry.rang)
      ) &&
      !(
